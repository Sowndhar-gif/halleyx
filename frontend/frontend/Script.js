// Full frontend script for customer and admin portals with AI and voice assistant integration
...
// Load customer products with pagination, search, sort
...
async function loadCustomerOrders() {
  if (!customerToken) return;
  try {
    const response = await fetch('http://localhost:3000/api/orders/mine', {
      headers: getAuthHeaders(customerToken)
    });
    const data = await response.json();
    if (!response.ok) {
...
document.getElementById('custProfileForm').addEventListener('submit', async function(e) {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/customers/${currentCustomer._id}`, {
      method: 'PUT',
      headers: getAuthHeaders(customerToken),
...
async function loadAdminDashboard() {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/settings/admin-dashboard', {
...
// Load admin products with pagination, search, sort
...
// Load admin customers with pagination, search, filter
...
// Load admin orders with pagination, filters
...
async function impersonateCustomer(customerId, customerName) {
  if (!adminToken) return;
  try {
    const response = await fetch(`http://localhost:3000/api/auth/impersonate/${customerId}`, {
      method: 'POST',
      headers: getAuthHeaders(adminToken)
    });
    const data = await response.json();
    ...
    // Switch to customer portal with impersonation token
...
function stopImpersonation() {
  ...
  // Reload page or reset to admin portal
  location.reload();
}
...
// View Customer Modal
async function viewAdminCustomer(customerId) {
  if (!adminToken) return;
  try {
    ...
    if (!response.ok) {
      alert(customer.message || 'Failed to load customer');
      return;
    }
    const content = `
      ...
      <p><strong>Status:</strong> ${customer.isBlocked ? 'Blocked' : 'Active'}</p>
      <button onclick="toggleBlockCustomer('${customer._id}', ${customer.isBlocked})">${customer.isBlocked ? 'Unblock' : 'Block'}</button>
      <button onclick="resetCustomerPassword('${customer._id}')">Reset Password</button>
...
async function resetCustomerPassword(customerId) {
  ...
  try {
    const response = await fetch(`http://localhost:3000/api/customers/${customerId}/reset-password`, {
      method: 'POST',
      headers: getAuthHeaders(adminToken)
    });
    const data = await response.json();
...
async function viewAdminOrder(orderId) {
  ...
  try {
    ...
    if (!response.ok) {
      alert(order.message || 'Failed to load order');
      return;
    }
    const content = `
      <h3>Order Details</h3>
      <p><strong>Order ID:</strong> ${order._id}</p>
      <p><strong>Customer:</strong> ${order.userId ? order.userId.firstName + ' ' + order.userId.lastName : ''}</p>
      <p><strong>Product:</strong> ${order.productId ? order.productId.name : ''}</p>
  ...
  try {
    ...
    const content = `
      ...
      <form id="updateOrderForm">
        ...
        <label for="updateOrderQuantity">Quantity</label>
        <input type="number" id="updateOrderQuantity" min="1" value="${order.quantity}" required />
        <label for="updateShippingAddress">Shipping Address</label>
        <textarea id="updateShippingAddress" required>${order.shippingAddress}</textarea>
        <label for="updateBillingAddress">Billing Address</label>
        <textarea id="updateBillingAddress" required>${order.billingAddress}</textarea>
        <button type="submit">Save Changes</button>
      </form>
    `;
    showModal(content);
    document.getElementById('updateOrderForm').addEventListener('submit', async function(e) {
      ...
      const quantity = parseInt(document.getElementById('updateOrderQuantity').value);
      const shippingAddress = document.getElementById('updateShippingAddress').value;
      const billingAddress = document.getElementById('updateBillingAddress').value;
      try {
        const res = await fetch(`http://localhost:5000/api/orders/${orderId}`, {
          method: 'PUT',
          headers: getAuthHeaders(adminToken),
          body: JSON.stringify({ status, quantity, shippingAddress, billingAddress })
        });
        const data = await res.json();
        if (!res.ok) {
